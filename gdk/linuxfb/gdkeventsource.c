...
void
_gdk_linuxfb_events_got_input (LinuxFbInputMsg *message)
{
  GdkDisplay *display;
  GdkLinuxFbDisplay *display_linuxfb;
  GdkLinuxFbDeviceManager *device_manager;
  GdkScreen *screen;
  GdkWindow *window;
  GdkEvent *event = NULL;
  GList *node;
  ...
  for (d = list; d; d = d->next)
    {
      if (GDK_IS_LINUXFB_DISPLAY (d->data))
        {
          display = d->data;
          break;
        }
  ...
  display_linuxfb = GDK_LINUXFB_DISPLAY (display);
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  device_manager = GDK_LINUXFB_DEVICE_MANAGER (gdk_display_get_device_manager (display));
  G_GNUC_END_IGNORE_DEPRECATIONS;

  switch (message->base.type) {
  case LINUXFB_EVENT_ENTER:
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->pointer.event_window_id));
    if (window)
      {
  ...
  case LINUXFB_EVENT_POINTER_MOVE:
    if (_gdk_linuxfb_moveresize_handle_event (display, message))
      break;
    ...
    break;
  case LINUXFB_EVENT_BUTTON_PRESS:
  case LINUXFB_EVENT_BUTTON_RELEASE:
    if (message->base.type != 'b' &&
	_gdk_linuxfb_moveresize_handle_event (display, message))
      break;
  ...
  case LINUXFB_EVENT_TOUCH:
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->touch.event_window_id));
    if (window)
      {
        ...
        case 2:
          ...
          break;
        default:
          g_printerr ("_gdk_linuxfb_events_got_input - Unknown touch type %d\n", message->touch.touch_type);
        }

        if (event_type != GDK_TOUCH_BEGIN &&
            message->touch.is_emulated && _gdk_linuxfb_moveresize_handle_event (display, message))
          break;
    ...
    break;
  case LINUXFB_EVENT_KEY_PRESS:
  case LINUXFB_EVENT_KEY_RELEASE:
    window = g_hash_table_lookup (display_linuxfb->id_ht,
    ...
    break;
  case LINUXFB_EVENT_GRAB_NOTIFY:
  case LINUXFB_EVENT_UNGRAB_NOTIFY:
    _gdk_display_device_grab_update (display, device_manager->core_pointer, device_manager->core_pointer, message->base.serial);
    break;

  case LINUXFB_EVENT_CONFIGURE_NOTIFY:
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->configure_notify.id));
    if (window)
      {
	...
	if (window->resize_count >= 1)
	  {
	    ...
	    if (window->resize_count == 0)
	      _gdk_linuxfb_moveresize_configure_done (display, window);
	  }
      }
    break;

  case LINUXFB_EVENT_DELETE_NOTIFY:
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->delete_notify.id));
    if (window)
      {
  ...
  case LINUXFB_EVENT_SCREEN_SIZE_CHANGED:
    ...
    _gdk_linuxfb_screen_size_changed (screen, &message->screen_resize_notify);
    break;

  case LINUXFB_EVENT_FOCUS:
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->focus.old_id));
    if (window)
      {
    ...
    window = g_hash_table_lookup (display_linuxfb->id_ht, GINT_TO_POINTER (message->focus.new_id));
    if (window)
      {
	event = gdk_event_new (GDK_FOCUS_CHANGE);
	event->focus_change.window = g_object_ref (window);
  ...
  default:
    g_printerr ("_gdk_linuxfb_events_got_input - Unknown input command %c\n", message->base.type);
    break;
  }
}

void
_gdk_linuxfb_display_queue_events (GdkDisplay *display)
{
}
