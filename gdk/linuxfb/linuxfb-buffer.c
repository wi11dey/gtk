...
static gboolean
verify_block_match (LinuxFbBuffer *buffer, int x, int y,
                    LinuxFbBuffer *prev, struct entry *entry)
{
  int i;
  void *old, *match;
  int w1, w2, h1, h2;
...
static void
insert_block (LinuxFbBuffer *buffer, guint32 h, int x, int y)
{
  struct entry *entry;
  int i;
  guint32 collision = 0;
...
static struct entry *
lookup_block (LinuxFbBuffer *prev, guint32 h)
{
  guint32 i;
  struct entry *entry;
...
void
linuxfb_buffer_destroy (LinuxFbBuffer *buffer)
{
  g_free (buffer->data);
  g_free (buffer->table);
  g_free (buffer);
...
int
linuxfb_buffer_get_width (LinuxFbBuffer *buffer)
{
  return buffer->width;
}

int
linuxfb_buffer_get_height (LinuxFbBuffer *buffer)
{
...
LinuxFbBuffer *
linuxfb_buffer_create (int width, int height, guint8 *data, int stride)
{
  LinuxFbBuffer *buffer;
  int y, bits_required;

  buffer = g_new0 (LinuxFbBuffer, 1);
  buffer->width = width;
...
void
linuxfb_buffer_encode (LinuxFbBuffer *buffer, LinuxFbBuffer *prev, GString *dest)
{
  struct entry *entry;
  int i, j, k;
  int x0, x1, y0, y1;
